...
──────────────────────────────────────────────────────────────────────────────────────────────
",     
    ANSI::Move(Loc::Home), ANSI::ClearScreen, ANSI::Bold,
    ANSI::Fore256(196),
    ANSI::Clear, ANSI::Bold,
    ANSI::Fore256(202),
    ANSI::Fore256(208),
    ANSI::Fore256(208),
    ANSI::Fore256(214),
    ...
    'outer: loop {
        let answer: String = possible_ans.choose(&mut rand::thread_rng()).unwrap().to_string();
        let c_freq: [u8; 27] = {
...
", ANSI::Move(Loc::Start));
        ...
        'words: while itr < 6 {
            ...
            stdout().flush();
            let mut attempt: String = get_word();
            // oh yay we found it
            // println!("{:#?}", answer);
            
            if attempt == "jank workaround lol dont mind me".to_string() { break; }
            match accepted_words.binary_search(&attempt) { 
                Err(_) => {
                    print!("{}***********", ANSI::Move(Loc::Word(itr))); 
                    continue; 
                },
            ...
            //println!("{:#?}", accepted_words.binary_search(&attempt));
            print!("{}", ANSI::Move(Loc::Word(itr)));            
            // mark the letters!
            let mut freq: [u8; 27] = c_freq.clone();
            for (i, letter) in attempt.chars().enumerate() {
                if letter as u8 == answer.as_bytes()[i] {
                    print!("{}{}", ANSI::Preset(Preset::Green), (letter as u8 - 32) as char);
            ...
            for (i, letter) in attempt.chars().enumerate() {
                if freq[letter as usize - 'a' as usize] > 0 && letter as u8 != answer.as_bytes()[i] {
                    print!("{}{}", ANSI::Preset(Preset::Yellow), (letter as u8 - 32) as char);
                    freq[letter as usize - 'a' as usize] -= 1;
                    continue;
                }
                print!("\x1b[1C");
            ...
            itr += 1;
            print!("{}", ANSI::Clear);
            if attempt == answer { break; }
        }
        print!("{}{}The word was: {}\npress Enter to continue\n{}", ANSI::Fore256(3), ANSI::Move(Loc::Bottom), answer, ANSI::Clear);
        let ch: Getch = Getch::new();
